...
export class PagesMethods extends Methods {
	...
	): Promise<WithPagination<'posts'>> {
		const result = await this.c.req(
	...
	async get(
		pageId: string,
		options: PagesGetOptions = {},
		{ signal }: { signal?: AbortSignal } = {},
	): Promise<Post | null> {
		assertNonBlankString(pageId, "Argument 'pageId'");

		const { posts } = await this.c.req(
			`./pages/${options.summary === true ? 'summary' : 'default'}/${encodeURI(pageId)}`,
			{
				exclude: ['query'],
				signal,
			},
