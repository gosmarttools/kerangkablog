...
function getPostComments(linkArray: unknown): PostCommentInfo {
	...
	const replies = getLinks(linkArray).links.filter(
		(link) => link.rel === 'replies',
	);
	if (replies) {
		for (const { title, type, href } of replies) {
			if (type === 'text/html' && isString(title)) {
				...
				result.title = title;
				result.number = numberMatches?.[0]
					? Number.parseInt(numberMatches[0], 10)
					: 0;
	...
	if (
		...
		for (let i = 0; i < extendedArray.length; i += 1) {
			...
			if (isString(name) && isString(value)) {
				const data: Record<
					string,
					['class' | 'time' | 'removed', string | boolean]
				> = {
...
function getThumbnail(postEntry: unknown): [string | null, string | null] {
	...
	const thumbnail: string | null = isString(postMediaThumbnailLike)
...
function getComment(commentEntry: unknown): Comment | null {
	...
	if (
		...
		const inReplyToMatches = links
			.find((link) => link.rel === 'related')
