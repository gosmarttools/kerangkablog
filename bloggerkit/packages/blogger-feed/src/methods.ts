...
export interface WithPagination<T extends keyof PaginationItemsMap> {
	...
	readonly next: (requestOptions?: {
		signal?: AbortSignal;
	}) => Promise<WithPagination<T> | null>;
	readonly previous: (requestOptions?: {
		signal?: AbortSignal;
	}) => Promise<WithPagination<T> | null>;
}
...
export class Methods {
	...
	protected _paginate<T extends keyof PaginationItemsMap>(
		type: T,
		feed: Feed,
	): WithPagination<T> {
		return {
			items: ((type === 'comments' ? feed.comments : feed.posts) ??
				[]) as PaginationItemsMap[T],
