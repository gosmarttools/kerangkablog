...
export class Client {
	...
	constructor(urlOrId: string | URL, options: ClientOptions = {}) {
		if (isString(urlOrId) && /^\d{12,24}$/.test(urlOrId)) {
			...
			if (!url) {
				throw new Error(
					"Argument 'urlOrId' is not a valid blogger blog url or blog id",
				);
			}
			if (!/^https?:$/i.test(url.protocol)) {
				throw new Error(
					`Argument 'urlOrId' has unsupported protocol '${url.protocol}'`,
				);
			}
		...
		if (
			...
			throw new Error(
				"options.jsonp is set to true but current environment does't support it, please set it to false to use json",
			);
		}
	}
