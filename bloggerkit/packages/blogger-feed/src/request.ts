...
async function fetchJSON<T = unknown>(
	...
	if (!contentType) {
		await response.body?.cancel();
		throw new SDKRequestError(
			`Request was success but Content-Type '${contentType}' is not supported`,
			response.url,
		);
	}
 ...
 */
async function fetchJSONP<T = unknown>(
	getUrl: JSONPGetUrl,
	{ signal }: { signal?: AbortSignal } = {},
): Promise<T> {
	return new Promise<T>((res, rej) => {
		...
		script.onload = () => {
			...
			if (!settled) {
				reject(
					new SDKError(
						`JSONP callback \`${callback}\` was not invoked for '${script.src}'`,
					),
				);
			}
		};
...
export async function fetchFeed(
	path: string | URL,
	{ params, include, exclude, base, jsonp, signal }: FetchFeedOptions = {},
): Promise<Feed> {
	...
	if (params) {
		for (const key in params) {
			...
			const allowed =
				(!include || (include as string[]).includes(key)) &&
				(!exclude || !(exclude as string[]).includes(key));
